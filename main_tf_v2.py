...
with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
        ...
        for step in range(max_steps):
            ...
            loss_value, summary_str = sess.run([sum_loss, summary])
        ...
        coord.request_stop()
        coord.join(threads)
    # tf.train.MonitoredTrainingSession(sum_loss,  # 该位置为损失函数tensor

    # slim.learning.train(sum_loss,
    #                     logdir='./logs/',
    #                     master='',
    #                     is_chief=True,
    #                     init_fn=util_tf.get_init_fn(checkpoint_path=None,  # './checkpoints/ssd_300_vgg.ckpt',
    #                                                 train_dir='./logs/',
    ...
    #                     log_every_n_steps=1,                            # 每次model保存step间隔
